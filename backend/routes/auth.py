...
def forgot_password():
    ...
    generic = {"message": "If that email is registered, a reset link has been sent to your inbox."}
    ...
    # When email cannot be delivered, return the link so the flow stays testable (local / FYP).
    if not sent:
        response["reset_link"] = reset_link
        response["message"] = (
            "We could not send email right now. Use this reset link to set a new password."
        )
